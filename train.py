...
class AugmentedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, images, labels, train: bool = True):
        self.images = images
        self.labels = labels
        # Only fast, permitted augmentations (no RandomAffine – too slow per-image)
        self.transform = (
            T.Compose([
...
# ================================================================
@torch.no_grad()
def estimate_target_priors(model, C_true, images, device,
                           batch_size=256, K=NUM_CLASSES):
    """
...
@torch.no_grad()
def predict_with_prior(model, images, device, p_target,
                       batch_size=256, K=NUM_CLASSES):
    """
...
# ================================================================
def generate_submission(model, C_true, base_state, device,
                        static_path, suite_path, out_path=SUBMISSION_PATH):
    results = []
    ...
    for skey in sorted(k for k in suite if k.startswith("scenario")):
        ...
        print(f"  {skey}: {len(scenario_imgs)} imgs, "
              f"priors={p_scen.numpy().round(3)}")
...
def main():
    ...
    print(f"Config : EPOCHS={EPOCHS}  LR={LR}  GCE_Q={GCE_Q}  "
          f"BS={BATCH_SIZE}  NOISE={NOISE_RATE}")

    # ---- load source data ----
    print("\n[Phase 0] Loading data ...")
    src = torch.load(os.path.join(DATA_DIR, "source_toxic.pt"),
                     map_location="cpu")
    all_imgs, all_labs = src["images"], src["labels"]
    print(f"  Source : {all_imgs.shape}   label dist : "
    ...
    if phase1 is not None:
        ...
        save_phase_checkpoint("1_training", {
            "model_state": model.state_dict(),
        })

    # ---- sanity check ----
    san = torch.load(os.path.join(DATA_DIR, "val_sanity.pt"),
                     map_location="cpu")
    san_loader = DataLoader(TensorDataset(san["images"], san["labels"]),
                            batch_size=100)
    ...
    if phase2 is not None:
        ...
        save_phase_checkpoint("2_confusion", {
            "C_true": C_true,
        })
    ...
    generate_submission(
        model, C_true, base_state, DEVICE,
