...
- Phase 2 — Confusion matrix estimation & correction
  - Compute empirical noisy confusion matrix on held-out validation (from source noisy labels), correct it using the known symmetric noise transition matrix `T` (the challenge provides noise rate), then invert (with Tikhonov regularisation) to obtain `C_true` used for BBSE.
...
**Contact**
- For questions about this repository or to request assistance reproducing results, contact `Abhav Garg` (team lead) via the contest platform.
