...
def find_optimal_temperature(
    ...
    """
    Learn the optimal temperature T that minimises NLL on a *clean* validation
    set by optimising a single scalar parameter with LBFGS.

    This is the standard post-hoc calibration method from:
        Guo et al., "On Calibration of Modern Neural Networks", ICML 2017.

    No noise transition matrix is involved — the labels are assumed clean.
    The temperature is clamped to [t_min, t_max] after optimisation.
    """
    model.eval()
    ...
    # Learnable log-temperature (unconstrained optimisation, then exp + clamp)
    log_temp = nn.Parameter(torch.zeros(1))  # initialises T=1.0
    ...
    def closure():
        optimizer.zero_grad()
        temp = log_temp.exp()
        loss = nll_criterion(all_logits / temp, all_labels)
        loss.backward()
        return loss

    optimizer.step(closure)

    optimal_T = log_temp.exp().item()
    optimal_T = max(t_min, min(t_max, optimal_T))

    # Report final NLL at the chosen temperature
    with torch.no_grad():
        final_nll = nll_criterion(all_logits / optimal_T, all_labels).item()

    logging.info(f"    Optimal T={optimal_T:.3f}  (NLL={final_nll:.4f})")
    return optimal_T
